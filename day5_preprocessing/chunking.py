...
def create_chunks(approved_documents=None):
    ...
    with open(output_file, "w", encoding="utf-8") as file:
        ...
        for document_path in cleaned_folder.glob("*.md"):
            ...
            document_id = document_path.stem.split("_", 1)[0]

            # If approved documents were provided,
            # process only those documents.
            if (
                approved_documents is not None
                and document_id not in approved_documents
            ):
                continue
...
# Run chunking when this file is executed directly
